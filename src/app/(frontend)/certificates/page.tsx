    ...
    async function fetchData() {
      try {
        const [certsRes, pageRes] = await Promise.all([
          fetch('/api/certificates?limit=100&sort=order').then((r) => r.json()),
    ...
    <>
      ...
      <section className="section">
        <div className="container">
          {formattedCerts.length === 0 && !loading ? (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', background: 'var(--white)', borderRadius: 'var(--radius-2xl)', border: '1px solid var(--gray-200)' }}>
              <h3 style={{ fontSize: '1.25rem', color: 'var(--navy-900)', fontWeight: 800 }}>گواهینامه‌ای یافت نشد</h3>
              <p style={{ color: 'var(--gray-600)', marginTop: '0.5rem' }}>در حال حاضر گواهینامه‌ای در سامانه ثبت نشده است.</p>
            </div>
          ) : (
