...
import { Calendar, ChevronLeft, User, Clock, BookOpen } from 'lucide-react'
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description: page?.metaDescription || 'مرکز مقالات تخصصی، اخبار پروژه‌ها، تحلیلهای مهندسی و اطلاعیه‌های رسمی شرکت ضرغام صنعت اروند',
  }
}
...
export default async function NewsPage() {
  ...
  return (
    <>
      <PageHero
        title={cmsPage?.heroTitle || 'مقالات و اخبار تخصصی'}
        badge={cmsPage?.heroBadge || 'مرکز دانش و اطلاعات'}
        subtitle={cmsPage?.heroSubtitle || 'آخرین مقالات مهندسی، گزارشات پیشرفت پروژه و اخبار صنعت نفت، گاز و پتروشیمی'}
        breadcrumbs={[{ label: 'مقالات و اخبار' }]}
                ...
                <article key={item.id} className="card" style={{ background: 'var(--white)', borderRadius: 'var(--radius-2xl)', overflow: 'hidden', border: '1.5px solid var(--gray-200)', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 20px rgba(0,0,0,0.04)' }}>
                  ...
                  <div style={{ padding: '1.75rem', display: 'flex', flexDirection: 'column', gap: '0.85rem', flex: 1 }}>
                    ...
                    <div style={{ paddingTop: '0.75rem', borderTop: '1px solid var(--gray-100)', marginTop: 'auto' }}>
                      <Link href={`/news/${item.slug}`} className="btn btn--outline btn--sm" style={{ width: '100%', justifyContent: 'center' }}>
